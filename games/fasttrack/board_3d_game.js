    ...
    const boardReadyCheck = setInterval(() => {
        ...
        if (hr && hr.size > 0) {
            ...
            // Auto-start game in offline/debug mode (no start screen needed)
            const _urlP = new URLSearchParams(window.location.search);
            if (_urlP.has('offline') || _urlP.has('debug')) {
                console.log('🎮 Offline/debug mode — auto-starting game session...');
