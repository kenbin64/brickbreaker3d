...
// ═══════════════════════════════════════════════════════════════════
//  BOARD_TOPOLOGY — SINGLE SOURCE OF TRUTH for all hole definitions.
//  Every file that references holes MUST point here. Never re-derive.
//
//  Perimeter track: 14 holes × 6 players = 84 holes (clockwise order)
//  Per-player section (P = player index 0-5):
//    #  | Hole ID              | Description
//   ----+----------------------+----------------------------
//    1  | ft-P                 | FastTrack hole (pentagon)
//    2  | side-left-P-1        | Side left near FT
//    3  | side-left-P-2        |
//    4  | side-left-P-3        |
//    5  | side-left-P-4        | Side left near outer corner
//    6  | outer-P-0            | Left corner
//    7  | outer-P-1            |
//    8  | outer-P-2            | Safe zone entry
//    9  | outer-P-3            |
//   10  | home-P               | Right corner / winning hole
//   11  | side-right-P-4       | Side right near home
//   12  | side-right-P-3       |
//   13  | side-right-P-2       |
//   14  | side-right-P-1       | Side right near next FT
//
//  Next hole (#15) = ft-{(P+1)%6} — the next player's section.
//
//  Additional per-player holes (not on perimeter):
//    safe-P-1 .. safe-P-4    (4 safe zone holes, entry via outer-P-2)
//    hold-P-0 .. hold-P-3    (4 holding area holes)
//
//  Shared holes:
//    center                  (1 bullseye)
//
...
//    Safe zone:  24   (4 × 6)
//    Holding:    24   (4 × 6)
//    FastTrack:   6   (subset of perimeter, pentagon corners)
//    Bullseye:    1
//    Home:        6   (subset of perimeter, diamond markers)
//    Grand total: 133
// ═══════════════════════════════════════════════════════════════════
const BOARD_TOPOLOGY = Object.freeze({
    PLAYERS: 6,
    HOLES_PER_SECTION: 14,
    PERIMETER_HOLES: 84,         // 14 × 6
    SIDE_HOLES_PER_SIDE: 4,      // side-left 1-4, side-right 4-1
    OUTER_HOLES_PER_SECTION: 4,  // outer 0-3
    ...
    // Clockwise section sequence (offsets within a section of 14)
    //   0: ft-P, 1-4: side-left-P-{1..4}, 5-8: outer-P-{0..3},
    //   9: home-P, 10-13: side-right-P-{4..1}
    SECTION_OFFSETS: Object.freeze({
        FT: 0,
        SIDE_LEFT_START: 1,   // side-left-P-1
        SIDE_LEFT_END: 4,     // side-left-P-4
        OUTER_START: 5,       // outer-P-0
        OUTER_END: 8,         // outer-P-3
        SAFE_ZONE_ENTRY: 7,   // outer-P-2
        HOME: 9,              // home-P
        SIDE_RIGHT_START: 10, // side-right-P-4
        SIDE_RIGHT_END: 13    // side-right-P-1
    }),

    // ID format templates
    ID: Object.freeze({
        ft:        (p) => `ft-${p}`,
        sideLeft:  (p, h) => `side-left-${p}-${h}`,   // h = 1..4
        outer:     (p, h) => `outer-${p}-${h}`,        // h = 0..3
        home:      (p) => `home-${p}`,
        sideRight: (p, h) => `side-right-${p}-${h}`,   // h = 1..4
        safe:      (p, h) => `safe-${p}-${h}`,          // h = 1..4
        holding:   (p, h) => `hold-${p}-${h}`,          // h = 0..3
        center:    () => 'center'
    }),

    // Build the canonical 84-hole clockwise perimeter track
    buildPerimeterTrack() {
        const track = [];
        for (let p = 0; p < 6; p++) {
            track.push(`ft-${p}`);
            for (let h = 1; h <= 4; h++) track.push(`side-left-${p}-${h}`);
            for (let h = 0; h < 4; h++) track.push(`outer-${p}-${h}`);
            track.push(`home-${p}`);
            for (let h = 4; h >= 1; h--) track.push(`side-right-${p}-${h}`);
        }
        return track;
    }
});
