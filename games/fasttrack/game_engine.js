 ...
 * Do NOT re-derive topology constants — always refer to BOARD_TOPOLOGY.
 * ════════════════════════════════════════════════════════════
 *
 * ============================================================
...
class GameState {
    ...
    getTrackSequence(peg, player, direction) {
        ...
        // 4 holes on each side line (matches smart_peg.js canonical layout)
        // Clockwise order for each player section (0-5):
        // 1. ft-{p} (FastTrack pentagon hole)
        // 2. side-left-{p}-1→4 (from FT toward outer corner)
        // 3. outer-{p}-0→3 (along outer edge)
        // 4. home-{p} (diamond marker at right corner)
        // 5. side-right-{p}-4→1 (from home toward next FT)
        const buildOrderedTrack = () => {
            ...
            for (let p = 0; p < 6; p++) {
                ...
                // Side-left holes: from FT toward outer (1, 2, 3, 4) - 4 holes parallel to safe zone
                for (let h = 1; h <= 4; h++) {
                ...
                // Side-right holes: from home toward next FT (4, 3, 2, 1) - 4 holes parallel to safe zone
                for (let h = 4; h >= 1; h--) {
        ...
        if (isOnSafeZoneEntry && shouldEnterSafeZoneFromEntry) {
            ...
            if (pegsInSafeZone < 4) {
                ...
                // 4 holes per side (matches smart_peg.js)
        ...
        if (currentHole.type === 'fasttrack' && dir === 'backward') {
            ...
            // 4 holes per side (going backward/counter-clockwise)
            // First: side-right holes of previous section (1, 2, 3, 4 going backward = reverse of CW order 4,3,2,1)
            for (let h = 1; h <= 4; h++) {
            ...
            // Then: side-left of previous section (4, 3, 2, 1 going backward = reverse of CW order 1,2,3,4)
            for (let h = 4; h >= 1; h--) {
        ...
        if (currentHole.type === 'fasttrack' && peg.onFasttrack) {
            ...
            if (currentFtIdx === playerFtExitPosition && hasTraversed) {
                ...
                // Exit to side-left track from this ft-* hole (clockwise: 1,2,3,4)
                for (let h = 1; h <= 4; h++) {
        ...
        if (currentIdx === -1) {
            ...
            if (currentHole.type === 'home') {
                ...
                if (homeMatch) {
                    ...
                    // home-X is at index 9 + (homePlayerIdx * 14) in the ordered track
                    // Section: ft(0) + side-left-1..4(1-4) + outer-0..3(5-8) + home(9) + side-right-4..1(10-13)
                    const calculatedIdx = 9 + (homePlayerIdx * 14);
        ...
        if (currentHole.type === 'home') {
            ...
            if (sequence.length === 0) {
                ...
                if (homeMatch) {
                    const homePlayerIdx = parseInt(homeMatch[1]);
                    // Build the continuation from home: side-right clockwise (4,3,2,1)
                    for (let h = 4; h >= 1; h--) {
    ...
    getBackwardHole(peg, player) {
        ...
        // Build the ordered track (same as getTrackSequence) - 4 holes per side (matches smart_peg.js)
        const buildOrderedTrack = () => {
            const orderedTrack = [];
            for (let p = 0; p < 6; p++) {
                orderedTrack.push(`ft-${p}`);
    ...
    getPegProgress(peg, playerBoardPosition) {
        ...
        if (holeId.startsWith('outer-')) {
            ...
            if (match) {
                ...
                // 14 holes per section: ft(0) SL-1..4(1-4) outer-0..3(5-8) home(9) SR-4..1(10-13)
                const withinSection = 5 + outerIdx;  // 5-8
                const totalProgress = sectionOffset * 14 + withinSection;
        ...
        if (holeId.startsWith('side-right-')) {
            ...
            if (match) {
                ...
                // CW order: SR-4(10), SR-3(11), SR-2(12), SR-1(13)
                const withinSection = 14 - sideIdx;  // 10-13
                const totalProgress = sectionOffset * 14 + withinSection;
                return Math.min(79, Math.max(1, totalProgress));
            }
        }
        ...
        if (holeId.startsWith('side-left-')) {
            ...
            if (match) {
                const sidePos = parseInt(match[1]);
                const sideIdx = parseInt(match[2]);
                // CW order: SL-1(1), SL-2(2), SL-3(3), SL-4(4) — right after ft hole
                const sectionOffset = (sidePos - playerBoardPosition + 6) % 6;
                const withinSection = sideIdx;  // 1-4
                const totalProgress = sectionOffset * 14 + withinSection;
                return Math.min(79, Math.max(1, totalProgress));
            }
        }
    ...
    // 14 holes per section × 6 sections = 84 total (matches smart_peg.js)
    // Section layout per player P:
    //   ft-P (0) → side-left-P-1..4 (1-4) → outer-P-0..3 (5-8) → home-P (9) → side-right-P-4..1 (10-13)
    // Returns -1 for non-perimeter holes (safe zone, center, holding)
    getPerimeterIndex(holeId) {
        if (!holeId) return -1;

        // ft-P: at section P's first position = P*14
        if (holeId.startsWith('ft-')) {
            const ftNum = parseInt(holeId.replace('ft-', ''));
            return ftNum * 14;
        }

        // side-left-P-H: H goes 1→4 in CW order, indices = P*14 + H
        // side-left-P-1 = P*14 + 1 (near FT), side-left-P-4 = P*14 + 4 (near outer corner)
