...
function createPrivateAsGuest() {
    // Show a name/avatar prompt before logging in as guest so players
    // are not stuck with an unplayable random "Player_xxxx" identity.
    showGuestSetupModal('private');
}

// Show a small modal so guest players can pick a name and avatar
// before we fire guest_login.  pendingAction is set to 'private' or 'join'.
function showGuestSetupModal(action) {
    state.pendingAction = action;

    // Build avatar emoji list (a small curated set)
    const QUICK_AVATARS = [
        '😀','😎','🤩','😏','🥸','🧐','😤','🤖','👻','🎃',
        '🦊','🐺','🐱','🐶','🐸','🦁','🐯','🐻','🐼','🐨'
    ];
    ...
    const saved = {
        name:   localStorage.getItem('fasttrack_player_name') || '',
        avatar: localStorage.getItem('fasttrack_player_avatar') || '😀'
    };

    // Remove any stale copy
    const existing = document.getElementById('guest-setup-modal');
    if (existing) existing.remove();

    const modal = document.createElement('div');
    modal.id = 'guest-setup-modal';
    modal.className = 'modal-overlay';
    modal.style.display = 'flex';
    modal.innerHTML = `
        <div class="modal" style="max-width:360px;">
            <div class="modal-header">
                <h2 class="modal-title">👤 Choose your identity</h2>
                <button class="modal-close" onclick="document.getElementById('guest-setup-modal').remove()">×</button>
            </div>

            <div class="input-group">
                <label>Your Name</label>
                <input type="text" class="input" id="guestSetupName"
                       placeholder="Enter a name…" maxlength="20"
                       value="${saved.name.replace(/"/g, '&quot;')}">
            </div>

            <div style="margin-bottom:1rem;">
                <label style="display:block;margin-bottom:0.5rem;font-size:0.85rem;color:var(--text-secondary);">Avatar</label>
                <div id="guestAvatarPreview" style="font-size:2.5rem;text-align:center;margin-bottom:0.5rem;">${saved.avatar}</div>
                <div style="display:flex;flex-wrap:wrap;gap:6px;justify-content:center;">
                    ${QUICK_AVATARS.map(a =>
                        `<button onclick="selectGuestAvatar(this,'${a}')"
                                 style="font-size:1.5rem;background:${a===saved.avatar?'rgba(100,200,255,0.2)':'transparent'};
                                        border:2px solid ${a===saved.avatar?'var(--accent)':'transparent'};
                                        border-radius:8px;padding:4px;cursor:pointer;min-width:40px;min-height:40px;">${a}</button>`
                    ).join('')}
                </div>
            </div>

            <button class="btn btn-primary" style="width:100%;" onclick="confirmGuestSetup()">
                Continue →
            </button>
        </div>
    `;
    ...
    setTimeout(() => {
        const inp = document.getElementById('guestSetupName');
        if (inp) inp.focus();
    }, 50);
}

function selectGuestAvatar(btn, emoji) {
    // Highlight chosen button
    document.querySelectorAll('#guest-setup-modal button[onclick^="selectGuestAvatar"]').forEach(b => {
        b.style.background = 'transparent';
        b.style.borderColor = 'transparent';
    });
    btn.style.background = 'rgba(100,200,255,0.2)';
    btn.style.borderColor = 'var(--accent)';
    document.getElementById('guestAvatarPreview').textContent = emoji;
}

function confirmGuestSetup() {
    const nameInput = document.getElementById('guestSetupName');
    const name = nameInput ? nameInput.value.trim() : '';
    if (!name) {
        nameInput && nameInput.focus();
        nameInput && (nameInput.style.borderColor = 'var(--danger, #ef4444)');
        return;
    }

    const avatar = document.getElementById('guestAvatarPreview').textContent || '😀';

    // Persist for next visit
    localStorage.setItem('fasttrack_player_name', name);
    localStorage.setItem('fasttrack_player_avatar', avatar);

    document.getElementById('guest-setup-modal').remove();
...
function playOfflineWithAI() {
    // Send player to the setup screen to choose name, avatar, difficulty, and bot count
    // before launching the game — avoids defaulting to anonymous "Player 1".
