...
// ============================================================
// THEME: BILLIARD TABLE - Prohibition Era Cherry Wood Pool Hall
// Classic 1920s speakeasy pool hall with Kenneth Bingham paintings
// ============================================================

FastTrackThemes.register('billiard', {
    name: 'Billiard Table',
    description: 'Prohibition-era cherry wood pool hall with original paintings by Kenneth Bingham',

    // Store references for parallax
    _poolHallObjects: [],
    _paintingFrames: [],

    // Kenneth Bingham's paintings
    paintings: [
        '/games/assets/images/bringingintheherd.jpg',
        '/games/assets/images/walinginrain.jpg',
        '/games/assets/images/threelittlepigs.jpg',
        '/games/assets/images/chess.png'
    ],
    ...
    create: function(scene, THREE, manager) {
        ...
        // Warm prohibition-era ambiance
        scene.background = new THREE.Color(0x0a0604);

        // Cherry wood materials (rich reddish-brown)
        const cherryWoodMat = new THREE.MeshStandardMaterial({
            color: 0x4a1c1c, roughness: 0.3, metalness: 0.1 // Rich cherry
        });
        const darkCherryMat = new THREE.MeshStandardMaterial({
            color: 0x2d1010, roughness: 0.35, metalness: 0.15 // Dark cherry
        });
        const floorWoodMat = new THREE.MeshStandardMaterial({
            color: 0x3a2218, roughness: 0.55, metalness: 0.05 // Aged oak floor
        });
        const wallpaperMat = new THREE.MeshStandardMaterial({
            color: 0x2a1f18, roughness: 0.9, metalness: 0.0 // Vintage wallpaper
        });
        const wainscotMat = new THREE.MeshStandardMaterial({
            color: 0x3d1a1a, roughness: 0.4, metalness: 0.1 // Cherry wainscoting
        });
        const greenFeltMat = new THREE.MeshStandardMaterial({
            color: 0x0d4528, roughness: 0.95, metalness: 0.0 // Classic felt
        });
        const goldTrimMat = new THREE.MeshStandardMaterial({
            color: 0xd4af37, roughness: 0.25, metalness: 0.7 // Gold trim
        });
        const brassMat = new THREE.MeshStandardMaterial({
            color: 0xb8860b, roughness: 0.3, metalness: 0.8 // Brass fixtures
        });

        // === WOODEN FLOOR with aged oak planks ===
        const floorGeo = new THREE.PlaneGeometry(2800, 2200);
        ...
        // === POOL HALL WALLS with cherry wainscoting ===
        const wallHeight = 450;
        const wainscotHeight = 140;

        // Back wall - upper wallpaper
        const backWallGeo = new THREE.PlaneGeometry(2800, wallHeight - wainscotHeight);
        const backWall = new THREE.Mesh(backWallGeo, wallpaperMat);
        backWall.position.set(0, wainscotHeight + (wallHeight - wainscotHeight)/2 - 80, -1100);
        scene.add(backWall);
        this._poolHallObjects.push({ mesh: backWall, parallaxFactor: 0.012, basePos: backWall.position.clone() });
        manager._sceneObjects.push(backWall);

        // Back wall - cherry wainscoting (lower half)
        const backWainscotGeo = new THREE.PlaneGeometry(2800, wainscotHeight);
        const backWainscot = new THREE.Mesh(backWainscotGeo, wainscotMat);
        backWainscot.position.set(0, wainscotHeight/2 - 80, -1098);
        scene.add(backWainscot);
        manager._sceneObjects.push(backWainscot);

        // Ornate chair rail (between wainscot and wallpaper)
        const chairRailGeo = new THREE.BoxGeometry(2800, 8, 12);
        const chairRail = new THREE.Mesh(chairRailGeo, cherryWoodMat);
        chairRail.position.set(0, wainscotHeight - 76, -1092);
        scene.add(chairRail);
        manager._sceneObjects.push(chairRail);

        // Gold trim on chair rail
        const chairRailTrimGeo = new THREE.BoxGeometry(2800, 3, 2);
        const chairRailTrim = new THREE.Mesh(chairRailTrimGeo, goldTrimMat);
        chairRailTrim.position.set(0, wainscotHeight - 72, -1088);
        scene.add(chairRailTrim);
        manager._sceneObjects.push(chairRailTrim);

        // Side walls with same treatment
        const sideWallGeo = new THREE.PlaneGeometry(2200, wallHeight - wainscotHeight);
        const leftWall = new THREE.Mesh(sideWallGeo, wallpaperMat);
        leftWall.position.set(-1350, wainscotHeight + (wallHeight - wainscotHeight)/2 - 80, 0);
        leftWall.rotation.y = Math.PI / 2;
        scene.add(leftWall);
        this._poolHallObjects.push({ mesh: leftWall, parallaxFactor: 0.015, basePos: leftWall.position.clone() });
        manager._sceneObjects.push(leftWall);

        const rightWall = new THREE.Mesh(sideWallGeo, wallpaperMat);
        rightWall.position.set(1350, wainscotHeight + (wallHeight - wainscotHeight)/2 - 80, 0);
        rightWall.rotation.y = -Math.PI / 2;
        scene.add(rightWall);
        this._poolHallObjects.push({ mesh: rightWall, parallaxFactor: 0.015, basePos: rightWall.position.clone() });
        manager._sceneObjects.push(rightWall);

        // Side wainscoting
        const sideWainscotGeo = new THREE.PlaneGeometry(2200, wainscotHeight);
        const leftWainscot = new THREE.Mesh(sideWainscotGeo, wainscotMat);
        leftWainscot.position.set(-1348, wainscotHeight/2 - 80, 0);
        leftWainscot.rotation.y = Math.PI / 2;
        scene.add(leftWainscot);
        manager._sceneObjects.push(leftWainscot);

        const rightWainscot = new THREE.Mesh(sideWainscotGeo, wainscotMat);
        rightWainscot.position.set(1348, wainscotHeight/2 - 80, 0);
        rightWainscot.rotation.y = -Math.PI / 2;
        scene.add(rightWainscot);
        manager._sceneObjects.push(rightWainscot);

        // === ORNATE CROWN MOLDING ===
        const crownGeo = new THREE.BoxGeometry(2800, 20, 20);
        const backCrown = new THREE.Mesh(crownGeo, cherryWoodMat);
        backCrown.position.set(0, wallHeight - 85, -1085);
        scene.add(backCrown);
        manager._sceneObjects.push(backCrown);

        // Gold crown trim
        const crownTrimGeo = new THREE.BoxGeometry(2800, 5, 3);
        const backCrownTrim = new THREE.Mesh(crownTrimGeo, goldTrimMat);
        backCrownTrim.position.set(0, wallHeight - 75, -1078);
        scene.add(backCrownTrim);
        manager._sceneObjects.push(backCrownTrim);

        // === PROPER POOL TABLE STRUCTURE ===
        const boardRadius = 260;
        const tableHeight = -5; // Board sits at y=0, table structure below

        // Table apron (thick wood frame under the board)
        const apronHeight = 30;
        const apronThickness = 25;
        for (let i = 0; i < 6; i++) {
            const angle1 = (i * Math.PI / 3) - Math.PI / 6;
            const angle2 = ((i + 1) * Math.PI / 3) - Math.PI / 6;

            const x1 = Math.cos(angle1) * (boardRadius + 5);
            const z1 = Math.sin(angle1) * (boardRadius + 5);
            const x2 = Math.cos(angle2) * (boardRadius + 5);
            const z2 = Math.sin(angle2) * (boardRadius + 5);
            ...
            // Apron (under board)
            const apronGeo = new THREE.BoxGeometry(length + 10, apronHeight, apronThickness);
            const apron = new THREE.Mesh(apronGeo, darkWoodMat);
            apron.position.set(midX, tableHeight - apronHeight/2, midZ);
            apron.rotation.y = -angle;
            apron.castShadow = true;
            scene.add(apron);
            manager._sceneObjects.push(apron);
        }

        // Table legs (6 ornate legs at corners)
        const legHeight = 45;
        for (let i = 0; i < 6; i++) {
            const angle = (i * Math.PI / 3) - Math.PI / 6;
            const x = Math.cos(angle) * (boardRadius + 15);
            const z = Math.sin(angle) * (boardRadius + 15);

            // Main leg
            const legGeo = new THREE.CylinderGeometry(12, 15, legHeight, 8);
            const leg = new THREE.Mesh(legGeo, darkWoodMat);
            leg.position.set(x, tableHeight - apronHeight - legHeight/2, z);
            leg.castShadow = true;
            scene.add(leg);
            manager._sceneObjects.push(leg);

            // Leg foot (decorative)
            const footGeo = new THREE.SphereGeometry(10, 8, 6, 0, Math.PI * 2, 0, Math.PI / 2);
            const foot = new THREE.Mesh(footGeo, darkWoodMat);
            foot.position.set(x, tableHeight - apronHeight - legHeight, z);
            foot.rotation.x = Math.PI;
            scene.add(foot);
            manager._sceneObjects.push(foot);
        }

        // === RAISED RAILS (cushions) - flush against board edge ===
        const railHeight = 18;
        const railWidth = 22;
        const cushionMat = new THREE.MeshStandardMaterial({
            color: 0x0a3d1f, roughness: 0.7, metalness: 0.0 // Darker green rubber
        });
        ...
        for (let i = 0; i < 6; i++) {
            ...
            // Rail positioned exactly at board edge
            const railRadius = boardRadius + railWidth/2 - 2;
            const x1 = Math.cos(angle1) * railRadius;
            const z1 = Math.sin(angle1) * railRadius;
            const x2 = Math.cos(angle2) * railRadius;
            const z2 = Math.sin(angle2) * railRadius;
            ...
            // Cushion (rubber part)
            const cushionGeo = new THREE.BoxGeometry(length - 20, railHeight * 0.6, railWidth * 0.5);
            const cushion = new THREE.Mesh(cushionGeo, cushionMat);
            cushion.position.set(midX, railHeight * 0.3 + 3, midZ);
            cushion.rotation.y = -angle;
            scene.add(cushion);
            manager._sceneObjects.push(cushion);

            // Wood rail cap
            const railCapGeo = new THREE.BoxGeometry(length, railHeight * 0.4, railWidth);
            const railCap = new THREE.Mesh(railCapGeo, darkWoodMat);
            railCap.position.set(midX, railHeight * 0.8 + 3, midZ);
            railCap.rotation.y = -angle;
            railCap.castShadow = true;
            scene.add(railCap);
            manager._sceneObjects.push(railCap);
        }

        // === TIFFANY-STYLE HANGING LAMP ===
        const lampY = 320;

        // Lamp shade (stained glass look)
        const shadeGeo = new THREE.ConeGeometry(100, 50, 8, 1, true);
        const shadeMat = new THREE.MeshStandardMaterial({
            color: 0x2d5a27, emissive: 0x1a3d15, emissiveIntensity: 0.3,
            roughness: 0.5, side: THREE.DoubleSide, transparent: true, opacity: 0.9
        });
        const lampShade = new THREE.Mesh(shadeGeo, shadeMat);
        lampShade.position.set(0, lampY, 0);
        lampShade.rotation.x = Math.PI;
        scene.add(lampShade);
        manager._sceneObjects.push(lampShade);
        this._poolHallObjects.push({ mesh: lampShade, parallaxFactor: 0.008, basePos: lampShade.position.clone() });

        // Brass fittings
        // Reuse brassMat from above (line ~957)
        const fittingGeo = new THREE.CylinderGeometry(15, 20, 15, 8);
        const fitting = new THREE.Mesh(fittingGeo, brassMat);
        fitting.position.set(0, lampY + 30, 0);
        scene.add(fitting);
        manager._sceneObjects.push(fitting);

        // Chain
        for (let c = 0; c < 8; c++) {
            const linkGeo = new THREE.TorusGeometry(4, 1.5, 8, 12);
            const link = new THREE.Mesh(linkGeo, brassMat);
            link.position.set(0, lampY + 50 + c * 15, 0);
            link.rotation.x = c % 2 === 0 ? 0 : Math.PI / 2;
            scene.add(link);
            manager._sceneObjects.push(link);
        }

        // Warm spotlight
        const lampLight = new THREE.SpotLight(0xffeedd, 2.0, 500, Math.PI / 2.5, 0.6, 1);
        lampLight.position.set(0, lampY - 20, 0);
        lampLight.target.position.set(0, 0, 0);
        lampLight.castShadow = true;
        scene.add(lampLight);
        scene.add(lampLight.target);
        manager._sceneObjects.push(lampLight);

        // Dim ambient (smoky atmosphere)
        const ambient = new THREE.AmbientLight(0x2a1a10, 0.25);
        scene.add(ambient);
        manager._sceneObjects.push(ambient);

        // === PAINTING FRAMES ON WALLS (placeholders for user art) ===
        const frameMat = new THREE.MeshStandardMaterial({ color: 0x3d2a1a, roughness: 0.4 });
        const paintingPositions = [
            { x: -400, y: 220, z: -940, w: 180, h: 140 },
            { x: 200, y: 200, z: -940, w: 220, h: 160 },
            { x: -1140, y: 200, z: -300, w: 160, h: 200, rotY: Math.PI/2 },
            { x: -1140, y: 220, z: 300, w: 180, h: 130, rotY: Math.PI/2 },
        ];

        paintingPositions.forEach((p, idx) => {
            // Frame
            const frameThickness = 12;
            const frameGeo = new THREE.BoxGeometry(p.w + frameThickness*2, p.h + frameThickness*2, 8);
            const frame = new THREE.Mesh(frameGeo, frameMat);
            frame.position.set(p.x, p.y, p.z);
            if (p.rotY) frame.rotation.y = p.rotY;
            scene.add(frame);
            manager._sceneObjects.push(frame);
            this._poolHallObjects.push({ mesh: frame, parallaxFactor: 0.012, basePos: frame.position.clone() });

            // Canvas (placeholder - will be replaced with actual paintings)
            const canvasMat = new THREE.MeshBasicMaterial({ color: 0x2a2a2a });
            const canvasGeo = new THREE.PlaneGeometry(p.w, p.h);
            const canvas = new THREE.Mesh(canvasGeo, canvasMat);
            canvas.position.set(p.x, p.y, p.z + 5);
            if (p.rotY) canvas.rotation.y = p.rotY;
            canvas.userData.paintingIndex = idx;
            scene.add(canvas);
            manager._sceneObjects.push(canvas);
            this._paintingFrames.push(canvas);
        });

        // === BACKGROUND ELEMENTS ===
        // Second pool table in distance
        const bgTableGeo = new THREE.BoxGeometry(200, 15, 100);
        const bgTable = new THREE.Mesh(bgTableGeo, greenFeltMat);
        bgTable.position.set(600, -25, -600);
        scene.add(bgTable);
        this._poolHallObjects.push({ mesh: bgTable, parallaxFactor: 0.04, basePos: bgTable.position.clone() });
        manager._sceneObjects.push(bgTable);

        // Cue rack on right wall
        const rackGeo = new THREE.BoxGeometry(8, 180, 40);
        const rack = new THREE.Mesh(rackGeo, medWoodMat);
        rack.position.set(1140, 140, 100);
        rack.rotation.y = -Math.PI/2;
        scene.add(rack);
        this._poolHallObjects.push({ mesh: rack, parallaxFactor: 0.025, basePos: rack.position.clone() });
        manager._sceneObjects.push(rack);

        // Pool cues
        const cueMat = new THREE.MeshStandardMaterial({ color: 0xd4a574 });
        for (let c = 0; c < 5; c++) {
            const cueGeo = new THREE.CylinderGeometry(1.5, 2.5, 150, 8);
            const cue = new THREE.Mesh(cueGeo, cueMat);
            cue.position.set(1135, 140, 80 + c * 12);
            cue.rotation.z = 0.08;
            scene.add(cue);
            manager._sceneObjects.push(cue);
        }

        // Chalk on rail
        const chalkGeo = new THREE.BoxGeometry(6, 6, 6);
        const chalkMat = new THREE.MeshStandardMaterial({ color: 0x4169e1 });
        const chalk = new THREE.Mesh(chalkGeo, chalkMat);
        chalk.position.set(boardRadius + 30, 25, 50);
        scene.add(chalk);
        manager._sceneObjects.push(chalk);

        console.log('🎱 [Billiard Theme] Golden age pool hall created');
        console.log('🖼️ [Billiard Theme] Painting frames ready:', this._paintingFrames.length);
    },

    // Set custom painting textures
        ...
        imageUrls.forEach((url, idx) => {
            if (this._paintingFrames[idx]) {
                loader.load(url, (texture) => {
                    ...
                    console.log(`🖼️ Painting ${idx + 1} loaded`);
                });
            }
        });
    },
