    ...
    function startSplitMoveMode() {
        ...
        // If only ONE active peg â€” normal 7-space move (no split, no wild card)
        if (activePegs.length === 1) {
            ...
            if (peg) {
                ...
                if (possibleMoves.length > 0) {
                    ...
                    cardUI.showMessage('Peg blocked â€” no valid moves!', 2000);
                    setTimeout(() => gameState.skipTurn(), 2000);
                }
            }
            return;
    ...
    function canSplitSeven() {
        ...
        // Rule: a single peg just moves exactly 7 spaces â€” no wild-card range.
        ...
        */
        ...
        gameState.onCardDrawn = (card) => {
            ...
            if (card.rank === '7') {
                ...
                if (!splitCheck.valid) {
                    // No valid moves at all â€” skip turn
                    console.log('[onCardDrawn] 7 Card â€” no valid moves possible, skipping turn');
                    legalMoves = [];
                    // Fall through to the legalMoves.length === 0 handler below
                } else if (splitCheck.singlePegOnly) {
                    // Only 1 peg on field â€” treat as a normal 7-space move (no split)
                    console.log('[onCardDrawn] 7 Card â€” single peg on field, treating as normal 7-space move');
            ...
            setTimeout(() => {
                ...
                function proceedToNextTurn() {
                    ...
                    function afterCameraReady() {
                        if (isAIPlayer(playerIdx)) {
                            // AI path â€” enable deck and take turn immediately
                            cardUI.setDeckEnabled(true);
                            aiTakeTurn();
                        } else {
                            // Human path â€” show prominent centered deck overlay
        ...
        gameState.onMoveExecuted = (move, cutPeg, entryFlags = {}) => {
            ...
            if (window.GameSFX) {
                ...
                if (entryFlags.exitedBullseye) {
                    ...
                    // Safe zone entry â€” play sound then do a little dance!
    ...
    function highlightLegalMoves(moves) {
        ...
        // MOVE SELECTION MODAL removed â€” destination holes glow on the board instead.
        ...
        // Show move suggestion panel on touch/mobile only â€” on desktop the lit holes
        ...
        if (GAME_CONFIG.showHighlights) {
            ...
            moves.forEach((move, idx) => {
                ...
                if (!hole) {
                    console.error(`âŒ [highlightLegalMoves] HOLE NOT FOUND: ${move.toHoleId}`);
                    missingCount++;
                    return;
                }
                if (!hole.mesh) {
                    console.error(`âŒ [highlightLegalMoves] HOLE HAS NO MESH: ${move.toHoleId}`);
                    missingCount++;
                    return;
                }
                ...
                // Create blinking highlight material with glow (use move's color; cuts shown by âš”ï¸ icon in panel)
        ...
        // Choice popup removed â€” players click/tap the glowing destination holes directly.
    ...
    // + MOBILE TOUCH ZONE â€” large invisible hitbox for easier tapping
    // ============================================================
    function createBullseyeTarget(hole, color) {
        ...
        // MOBILE TOUCH ZONE â€” large invisible sphere for easier tapping on small screens
            ...
            for (const [pegId, pegData] of pegRegistry) {
                if (pegData.isBlinking && 
                    ...
                    if (pegMoves.length >= 1) {
                        // Phase 1 â€” preview the first move for this peg (or the only move)
                        ...
                        console.log('[Click] Peg clicked â€” entering Phase 1 preview', pegMoves.length, 'moves');
                ...
                if (isHoleMesh || isTouchMesh) {
                    ...
                    if (hole.isHighlighted || isLegalDest) {
                        ...
                        if (movesForHole.length === 1) {
                            ...
                            // Ambiguous â€” require confirmation/choice
    ...
    function executeMoveDirectly(move) {
        ...
        // Safe to dismiss card overlay â€” move WILL execute (all guards passed)
        hideCenteredDeck();
    ...
    function animatePegMove(move, onComplete) {
        ...
        // Focus camera on moving peg before hops â€” respects the 3 camera modes:
        //   â€¢ Auto / Temp-explore (_tempManualOverride): ALWAYS follow the action.
        //     Temp explore is per-turn; the user still needs to see peg moves.
        //   â€¢ Explicit lock (fixedViewsActive / straight-down / angled):
        //     User chose this deliberately â€” camera never moves, even for peg animation.
        if (fixedViewsActive) {
            // Explicit lock: start hops immediately without moving camera
            // (legacy _allowCameraWrite removed â€” CameraDirector handles camera)
            beginHopsAfterFocus();
        } else {
    ...
    function performSafeZoneDance(move) {
        ...
        function animateSafeDance() {
            ...
            // Happy spin (faster than vanquish â€” it's a joyful move)
    ...
    // ============================================================
    // CENTERED DECK UI  â€” golden-ratio card, flip animation, fly-to-panel
    // ============================================================
    ...
    (function injectCenteredDeckCSS() {
        ...
        s.textContent = `
            #centered-deck-overlay {
                ...
                align-items: center; justify-content: center;
                gap: 0;
                animation: cdkFadeIn 0.45s ease-out both;
            }
            ...
            /* Wrapper handles perspective + bounce without fighting flip transform */
            #centered-deck-wrapper {
                position: relative;
                width: clamp(130px, 24vw, 185px);
                height: clamp(210px, 39vw, 299px);
                perspective: 900px;
            ...
            /* The actual card â€” rotates on Y for flip */
            #centered-deck-card {
    ...
    function showCenteredDeck(playerIdx, isExtraTurn) {
        ...
        banner.textContent = isExtraTurn
            ? `${playerAvatar} EXTRA TURN â€” ${playerName}`
    ...
    function updateCenteredDeckRank(card) {
        ...
        // (was 1800ms â€” cut to 600ms so the board appears immediately after rank shows)
        setTimeout(() => {
    ...
    // ================================================================
    // MOVE SUGGESTION PANEL â€” numbered, color-coded buttons, 2-phase UX
    // ================================================================
    ...
    function updateReplayVotes(votes) {
        ...
        if (allAgreed && Object.keys(votes).length >= totalHumanPlayers) {
            ...
            if (waitingDiv) {
                waitingDiv.innerHTML = `
                    <div style="color: #ff6b6b; margin-bottom: 10px;">âŒ Replay declined</div>
        ...
        if (notification && avatarEl && nameEl) {
            ...
            if (slotOptions && slotInfo) {
                if (availableSlots.hasBotSlot && availableSlots.hasEmptySlot) {
                    ...
                    slotOptions.style.display = 'none';
                    slotInfo.style.display = 'block';
                    slotInfo.innerHTML = 'âž• Will start with fresh pegs in slot ' + (availableSlots.emptySlotIndex + 1);
                } else {
    ...
    function startGameSession() {
        ...
        if (gameState) {
            ...
            if (isMultiplayerGame && _mpRoster && _mpRoster.length > 0) {
                ...
                // Solo / quickplay â€” assign AI bot identities from the manifold pool
