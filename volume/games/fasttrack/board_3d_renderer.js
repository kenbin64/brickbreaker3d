    ...
    // Per-move suggestion colors â€” vivid complementary hues, one per peg group
    ...
    function createPeg(id, playerIndex, holeId, colorIndex = null) {
        ...
        try {
            ...
            // render passes entirely â€” it will NOT appear as an "orb" and will NOT punch holes in
    ...
    function init() {
        ...
        // CameraGuard REMOVED â€” the new CameraDirector handles all camera
        // positioning through a single update() call each frame.
        // No monkey-patching of camera.position.set / lookAt needed.

        // Default camera mode is MANUAL (angled board view) â€” user has full control
        ...
        try {
            ...
            console.error('âŒ createRainbowBorder failed:', e);
        }
        ...
        console.log('âœ… init() completed, holeRegistry.size =', holeRegistry.size);
            ...
            console.error('âŒ init() error:', e);
    ...
    function createHexagonBoard() {
        ...
        // Layered wood grain rings â€” lighter and darker streaks
    ...
    function createRainbowBorder() {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            // Calculate exact edge length â€” no overlap
    ...
    function drawSchemaLines() {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            // Distance from outer edge to inner hex radius is about: outerRadius - innerHexRadius â‰ˆ 260 - 115 = 145
            // We need 6 side holes, so spacing should be about 145/7 â‰ˆ 20.7
        ...
        // ============================================================
        // GOLDEN CROWN â€” Glowing crown above WINNER hole when all 4 safe zone holes filled
        // ============================================================
    ...
    // ============================================================
    // CAMERA DIRECTOR â€” single centralized camera controller
    ...
    const CameraDirector = {
        ...
        update: function(dt) {
            ...
            // Skip in manual/pegeye modes â€” OrbitControls handles it
        ...
        _computeAutoTarget: function() {
            ...
            if (positions.length === 0) {
                // No pegs on board â€” show the whole board
            ...
            // Dynamic angle: 33Â° (close-up) to 66Â° (wide view), rule-of-thirds range
        ...
        if (boardViewMode === 0) {
            ...
            if (camera && controls) {
                if (boardViewMode === 1) {
                    ...
                    // Angled view â€” restore normal up vector
                    camera.up.set(0, 1, 0);
    ...
    // Update camera during peg movement â€” now a no-op.
    ...
    // ============================================================
    // PRE-CINEMATIC CAMERA â€” called on card draw to position the
    ...
    function preCinematicCamera(playerIdx, legalMoves) {
        // CameraDirector already frames all active player pegs â€” no-op.
    }
    ...
    function setupCameraInteractionTracking() {
        ...
        // Wheel (zoom) â€” temp explore this turn, resets at next turn start
        ...
        // Multi-touch gestures â€” same temp explore behaviour
    ...
    function onUserCameraRelease() {
        ...
        // No snap-back timer â€” that was the source of the Catan frustration.
        if (_pointerMoved) {
            userOverrideCamera = true;
            _tempManualOverride = true;
            _pointerStart = null;
            _pointerMoved = false;
            // Camera stays exactly where user left it â€” no return timer.
            return;
        }

        // Explicit lock (fixed views button) â€” always stay put
        if (fixedViewsActive) {
            _pointerStart = null;
            return;
        }

        // Tap without drag (e.g. selecting a peg) â€” cancel any pending timer and
    ...
    function returnToViewDefault() {
        ...
        if (view.autoOrbit) {
            ...
            // Board view â€” return to default
            smoothCameraTransition(
    ...
    // focusOnChoices â€” CameraDirector auto-frames all active pegs.
    ...
    function panOutForBoardView() {
        // CameraDirector auto-frames all pegs â€” no manual pan-out needed.
    ...
    function resetCameraOverride() {
        // CameraDirector handles framing â€” just clear the temp override flag
        _tempManualOverride = false;
        userOverrideCamera = false;
    }
    ...
    // Explicit lock (fixedViewsActive) is NOT cleared here â€” only the user can release it.
